---
type: module
package: "@hamafx/shared"
path: "packages/shared/test/symbol-catalog.test.ts"
incoming: 0
outgoing: 3
connections: 3
risk: low
layer: core
tags: [type/module, hamafx-shared, layer/core]
aliases: [symbol-catalog.test]
---

# 📁 symbol-catalog.test

> **Module** · `@hamafx/shared` · `packages/shared/test/symbol-catalog.test.ts`


Module: packages/shared/test/symbol-catalog.test.ts


## 📊 Connections

| | Count |
|---|---|
| Incoming dependencies | 0 |
| Outgoing dependencies | 3 |
| Total connections | 3 |
| Risk level | **LOW** |
| Layer | `core` |
| Package tag | `#hamafx-shared` |


### 📤 Depends On (3)
[[vitest-installServerOnlyStub]] · [[index_tool-architecture-explorer|index]] · [[@hamafx-shared]]





## 🔍 Explore

- **Local Graph:** Right-click this file → "Open local graph"
- **Backlinks:** Open the right sidebar → "Backlinks"
- Use `#type/module` to find all Modules
- Use `#hamafx-shared` to find all files in this package

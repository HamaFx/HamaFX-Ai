---
type: table
package: "@kestrel/db"
path: "packages/db/src/schema/provider-health.ts"
incoming: 0
outgoing: 1
connections: 1
risk: low
layer: data
tags: [type/table, kestrel-db, layer/data]
aliases: [provider_health]
---

# 🗄️ provider_health

> **DB Table** · `@kestrel/db` · `packages/db/src/schema/provider-health.ts`


Database table: provider_health (5 columns)


## 📊 Connections

| | Count |
|---|---|
| Incoming dependencies | 0 |
| Outgoing dependencies | 1 |
| Total connections | 1 |
| Risk level | **LOW** |
| Layer | `data` |
| Package tag | `#kestrel-db` |


### 📤 Depends On (1)
[[provider-health-providerHealth]]





## 🔍 Explore

- **Local Graph:** Right-click this file → "Open local graph"
- **Backlinks:** Open the right sidebar → "Backlinks"
- Use `#type/table` to find all DB Tables
- Use `#kestrel-db` to find all files in this package

---
type: table
package: "@kestrel/db"
path: "packages/db/src/schema/billing.ts"
incoming: 0
outgoing: 1
connections: 1
risk: low
layer: data
tags: [type/table, kestrel-db, layer/data]
aliases: [billing_checkout_attempts]
---

# 🗄️ billing_checkout_attempts

> **DB Table** · `@kestrel/db` · `packages/db/src/schema/billing.ts`


Database table: billing_checkout_attempts (12 columns)


## 📊 Connections

| | Count |
|---|---|
| Incoming dependencies | 0 |
| Outgoing dependencies | 1 |
| Total connections | 1 |
| Risk level | **LOW** |
| Layer | `data` |
| Package tag | `#kestrel-db` |


### 📤 Depends On (1)
[[billing-planInterval]]





## 🔍 Explore

- **Local Graph:** Right-click this file → "Open local graph"
- **Backlinks:** Open the right sidebar → "Backlinks"
- Use `#type/table` to find all DB Tables
- Use `#kestrel-db` to find all files in this package

...
PGlite provides an embedded development database. Development auth, cron, and encryption secrets are generated and persisted under `.hamafx/dev-secrets.json`.
...
- Multi-user startup refusal: shared mode is intentionally disabled; keep `MULTI_USER_ENABLED=0` and `HAMAFX_ENABLE_RLS=0`.

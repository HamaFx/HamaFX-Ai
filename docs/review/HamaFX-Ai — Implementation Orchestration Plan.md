...
| 1.1     | Done | 2026-07-01 | `run_system_action` now checks `user.role === 'admin'` before any FRED fetch or `intermarket_resonance` write; focused tests added. `4abc4d6` |
| 1.2     | Done | 2026-07-01 | Added server-side mutation-intent screening for `set_alert`, `log_journal`, `share_snapshot`, and `run_system_action`; explicit share/system-action requests required. `4abc4d6` |
| 1.3     | Done | 2026-07-01 | Removed theatrical `cot_sync`, `flush_cache`, and `check_migrations`; schema + docs now expose only the real `resonance_sync` action. `4abc4d6` |

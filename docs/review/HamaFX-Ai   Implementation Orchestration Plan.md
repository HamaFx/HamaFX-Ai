...
| 4.1     | Done | — | Sliding-window rate limiter implemented in `packages/db/src/rate-limit.ts`. Uses two adjacent minute buckets weighted by elapsed fraction. `RateLimitResult` now includes `resetAt`. | Verified — code review |
| 4.2     | Done | — | `checkGlobalDailyBudget()` + `getGlobalDailySpend()` added to `cost.ts`. Wired into `agent.ts` before per-user reservation. Uses `MAX_GLOBAL_DAILY_USD` env (default 10× per-user cap). | Verified — code review |
| 4.3     | Done | — | Rate limiting added to 8 provider-quota routes: market/price (60/min), market/candles (60/min), market/indicators (30/min), market/structure (30/min), news (30/min), calendar (30/min), sentiment (20/min), decision-signals (30/min). | Verified — code review |
| 4.4     | Done | — | `rateLimitedResponse()` helper added to `api.ts` with `Retry-After` + `X-RateLimit-*` headers. push/subscribe, push/unsubscribe, admin/test-alert-email, admin/test-telegram converted to standard `{error:{code,message,requestId}}` envelope. Chat route updated to use helper. | Verified — code review |
| 4.5     | Done | — | `MAX_OUTPUT_TOKENS=4096` and `MAX_INPUT_CONTEXT_TOKENS=60000` constants added to `cost.ts`. `maxOutputTokens` wired into `streamText` call. `estimateTurnCostUsd()` added for model-aware per-turn estimates. Budget reservation now uses model-aware estimate. | Verified — code review |
| 4.6     | Done | — | `RATES` table in `cost.ts` expanded from 8 to 40+ entries covering every model in `BYOK_PROVIDERS`. Fallback changed from `{5,15}` to conservative `{15,75}` with warning log for unknown models. `rateKeyForModel()` handles more provider prefixes. | Verified — code review |
| 4.7     | Done | — | `collectFindings` in `verification.ts` upgraded from turn-level to value-level. `extractToolResultNumbers()` compares claimed prices against actual tool-result values. `PRICE_TOKEN` regex broadened for comma-formatted, integer, and JPY-style prices. `parsePriceToken()` helper added. | Verified — code review |
| 4.8     | Done | — | Domain/range bounds added to `AlertRuleSchema` (0.0001–100,000) and `log_journal` tool input (entry/stop/target: 0.0001–100,000, size: 0–10,000,000). | Verified — code review |
| 4.9     | Done | — | `spend-anomaly.ts` worker job created with z-score SQL from review 07 §3.4(b). Registered in worker job registry + `JobName` type. Cron route at `/api/cron/spend-anomaly`. Captures to Sentry + pages operator via email/Telegram. | Verified — code review |
| 4.10    | Done | — | Tenant=user confirmed; org_id skipped per progress log decision. `actualCostUsd` + `byokKey` columns added to `chat_telemetry` schema (migration 0040). `recordTelemetry()` passes BYOK flag from agent. | Verified — code review |
| 4.11    | Done | — | Dead domain→model routing path documented as removed in `routing.ts` header. Single `chatModel` used for all domains. Context caching documented as handled at provider/model level. | Verified — code review |

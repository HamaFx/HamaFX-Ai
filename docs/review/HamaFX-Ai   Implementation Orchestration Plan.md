...
| 3.4     | Partial | 2026-07-02 | Added direct/session URL preference in `drizzle.config.ts` and predeploy/backup scripts; added `withTenantDb()` + `SET LOCAL app.current_tenant` helper. `1728241` | ⚠ Not yet — partial only; independent verification still needed |
| 3.5     | Done | 2026-07-02 | Updated `searchMemory` to filter on `tenant_id` (while keeping `user_id`) and set HNSW `ef_search`, `iterative_scan`, and `max_scan_tuples` per query. `1728241` | ⚠ Not yet — Phase 3 requires independent verification by another session |
...
| 3.9     | Todo | 2026-07-02 | Not started in this session. Secrets-vault rollout for hosted Vercel + worker remains. | n/a — not done |
| 3.10    | Todo | 2026-07-02 | Not started in this session. Tenant namespacing for module-level caches remains. | n/a — not done |
| 3.11    | Todo | 2026-07-02 | Not started in this session. `__system__` fallback removal still remains across web + AI files. | n/a — not done |
| 3.12    | Todo | 2026-07-02 | Not started in this session. `SELECT * FROM users WHERE id = '__system__'` check/report still remains. | n/a — not done |

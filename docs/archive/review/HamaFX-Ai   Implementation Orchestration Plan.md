...
- **Phase 8.3 (resolved):** Stripe, Paddle, Lemon Squeezy, and PayPal do not support Iraq-based merchants. 2Checkout/Verifone docs are conflicting. **Decision: use NOWPayments (crypto payment processor)** — no country restrictions, supports subscription billing, payout via Binance P2P → IQD. See `docs/review/10-billing-nowpayments-integration-plan.md`.
...
| 6.1     | Done | 2026-07-02 | Created `hamafx-job-resonance-sync.service` + `.timer` (daily 23:00 UTC, `Persistent=true`). The job was registered in code with an HC UUID but had no systemd unit — it never ran on the VM. The `run_system_action` AI tool path (`resonance_sync`) is the same underlying sync (both call FRED + upsert `intermarket_resonance`); the systemd timer makes it run automatically, while the AI tool remains for operator-triggered on-demand runs. `setup.sh`'s glob pattern auto-installs the new units. | ⚠ Not yet — verify units exist on the VM after next `setup.sh` run |
| 6.2     | Done | 2026-07-02 | Added `embedding-backfill` to `SKIP_DAILY_LOCK` in `scheduler.ts`. The daily `acquireCronLock` combined with the 6-hour timer meant a failed 00:00 run blocked all 3 same-day retries. The job is already idempotent per-article (query filters `embedding IS NULL`), so removing the daily lock lets each 6-hour firing run independently. Updated scheduler test with new test case. | ⚠ Not yet — spot-check |
| 6.3     | Done | 2026-07-02 | Extended `update.sh` with a 30-second post-deploy health-check window after `systemctl restart`. Checks `systemctl is-active` (must be `active`) and `NRestarts` (must be ≤ 1). If the worker crashes or enters a restart loop within the window, `rollback()` fires automatically — no more manual SSH for runtime-crash deploys. | ⚠ Not yet — spot-check on VM |
...
| 8.1     | Done | 2026-07-02 | Ran Prompt 00 (Documentation & Reality Drift) as a subagent. 13 files edited, 20 corrections: fixed BiQuote API key→URL drift, expanded BYOK provider list 6→8, added 2 missing tools to README matrix, fixed table/migration counts in AGENTS.md and docs, added 7 missing env vars to .env.example, fixed stale `docs/06-data-sources.md` references → `docs/04-data-layer.md` in 4 code files + 1 doc. Changelog at `docs/review/00-documentation-fixes-changelog.md`. | ⚠ Not yet — spot-check |
| 8.2     | Done | 2026-07-02 | Ran Prompt 09 (Open-Core Architecture) as a subagent. Produced `docs/review/09-open-core-architecture-review.md` (758 lines). 51 single-tenant assumptions catalogued (F1–F51). Proposed `HAMAFX_EDITION` env var as config boundary, 3 new packages (billing, tenancy, admin), 12-step implementation plan. License analysis: Apache-2.0 vs PostHog/Plausible/Supabase models. 6 open questions for owner. | ⚠ Not yet — spot-check |
| 8.3     | Done | 2026-07-02 | Ran billing audit as a subagent. Researched Stripe, Paddle, Lemon Squeezy, 2Checkout/Verifone, PayPal, NOWPayments, BTCPay Server, CoinGate. **Owner decision: use NOWPayments (crypto)** — no country restrictions for Iraq-based merchant, supports subscription/recurring billing via API, 100+ cryptocurrencies, payout via Binance P2P → IQD. Produced `docs/review/10-billing-nowpayments-integration-plan.md` (replaces Verifone plan). Covers HMAC-SHA512 webhook verification, idempotency, hosted checkout flow, Drizzle schema (plans/subscriptions/payments/ipn_events), 6-phase implementation plan, payout path to Iraq. | ⚠ Not yet — spot-check |
| 8.4     | Done | 2026-07-02 | Ran Prompt 11 (Legal/Compliance) as a subagent. Produced `docs/review/11-legal-compliance-review.md` (894 lines). 5 technical gaps: [CRITICAL] no ToS/Privacy Policy/disclaimer anywhere, [HIGH] no ToS acceptance flow, [HIGH] internal "not financial advice" rule invisible to users, [MEDIUM] in-app export missing 12/25 tables, [MEDIUM] no refund/cancellation policy surface. 22 open legal questions separated for lawyer. Technical remediation plan with `[PENDING LEGAL TEXT]` placeholders. | ⚠ Not yet — spot-check; legal questions require lawyer |

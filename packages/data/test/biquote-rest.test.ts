...
describe('biquote fetchOhlc', () => {
  ...
  it('throws when bars arrive out of ascending-time order (upstream contract guard)', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      jsonResponse([
        bar('2026-05-27T18:02:00Z'),
        bar('2026-05-27T18:01:00Z'),
        bar('2026-05-27T18:00:00Z'),
      ]),
    ) as unknown as typeof fetch;
    ...
    await expect(
      ...
      code: 'PROVIDER_PARSE_ERROR',
      message: expect.stringContaining('ascending-time order') as unknown as string,
    });
  });

  it('caps `count` at 2000 to match the documented BiQuote limit', async () => {
    const fetchSpy = vi.fn().mockResolvedValue(
      jsonResponse([bar('2026-05-27T18:00:00Z')]),
    );
    globalThis.fetch = fetchSpy as unknown as typeof fetch;

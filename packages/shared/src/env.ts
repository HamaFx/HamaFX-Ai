 ...
 * Used by the dev-mode env autogen path. Default length is 32 bytes
 * (64 hex chars) — comfortably exceeds the 32-char min on the secret
 * schemas.
 */
 ...
 * Kept as a const tuple so callers (apps/web/src/lib/env.ts) can iterate
 * without string-typing the same names in two places.
 */

...
export function Chart({ symbol, tf, count = 300, heightClass = 'h-[60svh]', className }: ChartProps) {
  ...
  return (
    <div className={cn('border-border bg-bg-elev-1 relative overflow-hidden rounded-lg border', className)}>
      <div ref={containerRef} className={cn('w-full', heightClass)} aria-label={`${symbol} ${tf} chart`} />
      {isLoading ? (
